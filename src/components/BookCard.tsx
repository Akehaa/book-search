...
export function BookCard({ title, author, language, snippet, src }: BookCardProps) {
  ...
  return (
    <div className="flex w-[21rem] p-3 gap-4 mb-9 bg-[#171c30] rounded-lg">
      {!src
        ? <Image src={no_cover} width={100} height={150} alt="book cover" className="rounded-lg h-[9rem]" />
        : <Image src={src} width={100} height={150} alt="book cover" className="rounded-lg h-[9rem]" />
      }
      <div>
        ...
        <div className="flex flex-row  text-xs my-1">
          <span className="outline-1 outline outline-[#8486dd] rounded-md py-1 px-2">{language}</span>
        </div>
        {!snippet
          ? <p className="line-clamp-3 text-sm">This book has no description available.</p>
          : <p className="line-clamp-3 text-sm">{snippet}</p>}
      </div>
    </div>
  )
}